...
export default {
  ...
  props: {
    ...
    currentDate: {
      type: String,
      required: false, // 根据你的需求，如果不是必须可以设置为 false
    },
    thresholds: {
      type: Object,
      required: true
  ...
  methods: {
    ...
    shouldBeVisible(node) {
      ...
      const thresholdKey = typeToThresholdKey[node.group];
      const isTypeEnabled = typeToBoolean[node.group];

      // 检查结点是否应被显示基于其类型和得分阈值
      let isVisibleBasedOnTypeAndScore = isTypeEnabled && node.pageRankScore >= this.thresholds[thresholdKey];

      // 对于专利结点（假设它们的group是0），还需要检查公开日期
      if (node.group === 0) {
        // 日期比较：确保专利的公开日期是在this.currentDate之后
        // 假设公开日期和currentDate的格式都是'YYYY.MM.DD'
        const patentDate = new Date(node.公开日.split('.').join('-')); // 转换为'YYYY-MM-DD'格式，因为JS Date更好支持这种格式
        const currentDate = new Date(this.currentDate.split('.').join('-')); // 同上

        // 检查专利的公开日期是否在当前日期之后
        if (patentDate <= currentDate) {
          return isVisibleBasedOnTypeAndScore;
        } else {
          return false;
        }
      }
