      ...
      svg.selectAll("circle")
          ...
          .attr('fill', function (d) {
            ...
            const colorScale = d3.scaleLinear()
                .domain([1, 10]) // 假设pageRankScore范围为1到10
                .range(["#77f", "#f00"]); // 蓝色到红色的渐变
            return colorScale(Math.sqrt(d.pageRankScore));
          });
...
// 箭头定义
      ...
      const node = svg.append('g')
          ...
          .enter().append("circle")
          .attr("r", function (d) {
            // 根据pageRankScore开根号来计算大小，再乘以一个系数以适当放大
            return Math.sqrt(d.pageRankScore) * 3; // 调整因子可根据视觉效果调优
          })
          .attr('fill', function (d) {
            // 颜色映射，重要性越高颜色越"热"
            const colorScale = d3.scaleLinear()
      ...
      const text = svg.append('g')
          ...
          .attr("font-size", function (d) {
            // 使用 Math.sqrt 对 pageRankScore 开根号，并进行适当的缩放
            let baseSize = Math.sqrt(d.pageRankScore) * 2 - 2; // 缩放因子可以根据实际情况调整
            return baseSize;
          })
          .attr("fill", '#444') // 文本用黑色
          .attr('name', function (d) {
